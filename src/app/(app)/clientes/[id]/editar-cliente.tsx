...
export function EditarClienteBtn({ cliente, isAdmin }: Props) {
  ...
  async function salvar() {
    ...
    const input: AtualizarClienteInput = {
      ...
      responsavel_endereco_cep: respCep.trim() || null,
    }
  ...
  async function apagar() {
    if (!window.confirm(`Apagar o contratante "${cliente.razao_social}"?\n\nEle será marcado como excluído (soft delete) e não aparecerá mais nas listas.`)) return
    setApagando(true)
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Editar contratante" maxWidth={760}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <Section titulo="Empresa">
            <Input label="Razão social" value={razao} onChange={(e) => setRazao(e.target.value)} />
            <Grid>
              <Input label="CNPJ" value={cnpj} onChange={(e) => setCnpj(formatCnpj(e.target.value))} />
          ...
          <Section titulo="Endereço da empresa">
            <Grid>
              <Input label="CEP" value={cep} onChange={(e) => setCep(formatCep(e.target.value))} />
              <div />
            </Grid>
          ...
          <Section titulo="Responsável (vai pros placeholders {{responsavel_*}})">
            ...
            <Grid>
              <Input label="CPF" value={respCpf} onChange={(e) => setRespCpf(formatCpf(e.target.value))} />
          ...
          <Section titulo="Endereço residencial do responsável">
            <Grid>
              <Input label="CEP" value={respCep} onChange={(e) => setRespCep(formatCep(e.target.value))} />
              <div />
            </Grid>
